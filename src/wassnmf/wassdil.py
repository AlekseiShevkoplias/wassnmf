...
class WassersteinDiL:
    ...
    def ot_entropic_semidual(
        self, X: torch.Tensor, G: torch.Tensor, eps: float, K: torch.Tensor
    ) -> torch.Tensor:
    ...
    def get_primal_weights(
        self, D: torch.Tensor, G: torch.Tensor, rho1: float
    ) -> torch.Tensor:
        """Compute primal weights from dual variables."""
        return F.softmax(-D.T @ G / rho1, dim=0)

    def get_primal_dict(
        self, Lambda: torch.Tensor, G: torch.Tensor, rho2: float
    ) -> torch.Tensor:
