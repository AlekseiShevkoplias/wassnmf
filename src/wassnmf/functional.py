...
def ot_entropic_semidual(X: torch.Tensor, G: torch.Tensor, eps: float, K: torch.Tensor) -> torch.Tensor:
...
def ot_entropic_semidual_grad(X: torch.Tensor, G: torch.Tensor, eps: float, K: torch.Tensor) -> torch.Tensor:
    return K.T @ (X / (K @ torch.exp(G / eps))) * torch.exp(G / eps)


# Julia Equivalent: Dual.getprimal_ot_entropic_semidual
def getprimal_ot_entropic_semidual(mu: torch.Tensor, v: torch.Tensor, eps: float, K: torch.Tensor) -> torch.Tensor:
    return torch.exp(v / eps) * (K.T @ (mu / (K @ torch.exp(v / eps))))

# Julia Equivalent: Dual.ot_entropic_dual
def ot_entropic_dual(u: torch.Tensor, v: torch.Tensor, eps: float, K: torch.Tensor) -> torch.Tensor:
    return eps * torch.log(dot_vecwise(torch.exp(u / eps), K @ torch.exp(v / eps)))

# Julia Equivalent: Dual.ot_entropic_dual_grad
def ot_entropic_dual_grad(u: torch.Tensor, v: torch.Tensor, eps: float, K: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    U = torch.exp(u / eps)
    V = torch.exp(v / eps)
    KV = K @ V
...
def getprimal_ot_entropic_dual(u: torch.Tensor, v: torch.Tensor, eps: float, K: torch.Tensor) -> torch.Tensor:
    return K * add_singleton(torch.exp(-u / eps), 1) * add_singleton(torch.exp(-v / eps), 0)
...
def dual_obj_weights(X: torch.Tensor, K: torch.Tensor, eps: float, D: torch.Tensor, G: torch.Tensor, rho1: float) -> torch.Tensor:
    return ot_entropic_semidual(X, G, eps, K).sum() + rho1 * e_star(-torch.matmul(D.T, G) / rho1).sum()

def dual_obj_weights_grad(X: torch.Tensor, K: torch.Tensor, eps: float, D: torch.Tensor, G: torch.Tensor, rho1: float) -> torch.Tensor:
    return ot_entropic_semidual_grad(X, G, eps, K) - torch.matmul(D, e_star_grad(-torch.matmul(D.T, G) / rho1))

def dual_obj_dict(X: torch.Tensor, K: torch.Tensor, eps: float, Lambda: torch.Tensor, G: torch.Tensor, rho2: float) -> torch.Tensor:
    return ot_entropic_semidual(X, G, eps, K).sum() + rho2 * e_star(-torch.matmul(G, Lambda.T) / rho2).sum()


def dual_obj_dict_grad(X: torch.Tensor, K: torch.Tensor, eps: float, Lambda: torch.Tensor, G: torch.Tensor, rho2: float) -> torch.Tensor:
    return ot_entropic_semidual_grad(X, G, eps, K) - torch.matmul(e_star_grad(-torch.matmul(G, Lambda.T) / rho2), Lambda)
...
def solve_weights(X: torch.Tensor, K: torch.Tensor, eps: float, D: torch.Tensor, rho1: float, use_gpu: bool = False) -> torch.Tensor:
    ...
    def closure():
        ...
        loss.backward(retain_graph=True) # Keep the graph for subsequent backward passes
        return loss

    optimizer = LBFGS([G], max_iter=250, tolerance_grad=1e-4, line_search_fn='strong_wolfe')
    optimizer.step(closure)

    return getprimal_weights(D, G, rho1)

def solve_dict(X: torch.Tensor, K: torch.Tensor, eps: float, Lambda: torch.Tensor, rho2: float, use_gpu: bool = False) -> torch.Tensor:
    device = torch.device("cuda" if use_gpu else "cpu")
    X = X.to(device)
    K = K.to(device)
    Lambda = Lambda.to(device)

    G = torch.zeros_like(X, device=device)  # Initialize dual variable. Note: This G is *different* from the G in solve_weights
    G.requires_grad_(True) # Ensure G has gradient tracking enabled
...
def wasserstein_nmf(X: torch.Tensor, K: torch.Tensor, k: int, eps: float = 0.025, rho1: float = 0.05,
                    rho2: float = 0.05, n_iter: int = 10, verbose: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
...
def wasserstein_nmf_gpu(X: torch.Tensor, K: torch.Tensor, k: int, eps: float = 0.025,
