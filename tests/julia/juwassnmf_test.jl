...
@testset "JuWassNMF.jl" begin
    ...
    @testset "Full WNMF" begin
        X = generate_test_data(15, 8)
        k = 3
        
        # Test with default parameters
        D, Λ = JuWassNMF.wasserstein_nmf(X, k, verbose=false)
        @test size(D) == (15, k)
        @test size(Λ) == (k, 8)
        @test all(D .>= 0)
        @test all(Λ .>= 0)
        ...
        D, Λ = JuWassNMF.wasserstein_nmf(X, k,
            eps=0.1,
            rho1=1e-1,
            rho2=1e-1,
            n_iter=5,
    ...
    @testset "Edge Cases" begin
        ...
        D, Λ = JuWassNMF.wasserstein_nmf(X_tiny, 2, verbose=false)
        @test size(D) == (4, 2)
        @test size(Λ) == (2, 3)
        
        # Large regularization
        X_small = generate_test_data(10, 5)
        D, Λ = JuWassNMF.wasserstein_nmf(X_small, 2, eps=1.0, verbose=false)
        @test size(D) == (10, 2)
        @test size(Λ) == (2, 5)
    end
end