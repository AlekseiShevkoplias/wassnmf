...
def generate_test_data(n_features=100, n_samples=20):
    ...
    for i in range(n_samples):
        ...
        X[:, i] = (np.random.rand() * np.exp(-(x - (np.random.randn() + 6))**2) +
...
def test_different_sizes():
    ...
    small_data = generate_test_data(20, 10)
...
def test_reproducibility():
    ...
    # Results should be similar (not exactly equal due to optimization)
